...
export async function startSignIn(): Promise<void> {
  ...
  // Clerk forwards arbitrary search params through to the post-sign-in
  // redirect; we round-trip the state + challenge to /auth/desktop-handoff.
  ...
  if (TAURI_AVAILABLE) {
    const { openUrl } = await import('@tauri-apps/plugin-deep-link').catch(() => ({
...
async function stashPending(pkce: PendingPkce): Promise<void> {
  ...
  const store = await load(PENDING_STORE, { autoSave: false });
